...
void winTimerSub(struct timeval *a, struct timeval *b, struct timeval *res){
	int microseconds = (b->tv_sec - a->tv_sec) * 1000000 + ((int)b->tv_usec - (int)a->tv_usec);
	int milliseconds = microseconds / 1000;

	res->tv_sec = microseconds / 1000000;
	res->tv_usec = microseconds % 1000000;
}
...
int populateCipherList(struct sslCheckOptions *options, const SSL_METHOD *sslMethod)
{
	...
	if (options->ctx == NULL) {
		("%sERROR: Could not create CTX object.%s\n", COL_RED, RESET);
		return false;
	}
	SSL_CTX_set_cipher_list(options->ctx, "ALL:COMPLEMENTOFALL");
	ssl = SSL_new(options->ctx);
	if (ssl == NULL) {
		std::cerr << "%sERROR: Could not create SSL object.%s\n", COL_RED, RESET;
	...
	for (loop = 0; loop < sk_SSL_CIPHER_num(cipherList); loop++)
	{
		...
		{
			options->ciphers = (sslCipher*)malloc(sizeof(struct sslCipher));
		...
		else
		{
			...
			sslCipherPointer->next = (sslCipher*)malloc(sizeof(struct sslCipher));
			sslCipherPointer = sslCipherPointer->next;
		}
		// Init
...
int readOrLogAndClose(int fd, char* buffer, size_t len, const struct sslCheckOptions *options)
{
	ssize_t n;
	...
	if (n < 0 && errno != 11) {
		std::cerr << "%s    ERROR: error reading from %s:%d: %s%s\n", COL_RED, options->host, options->port, 0, RESET;
		closesocket(fd);
		return 0;
	}
	else if (n == 0) {
		std::cerr << "%s    ERROR: unexpected EOF reading from %s:%d%s\n", COL_RED, options->host, options->port, RESET;
		closesocket(fd);
		return 0;
	}
	else {
...
int tcpConnect(struct sslCheckOptions *options)
{
	...
	{
		std::cerr << "%s    ERROR: Could not open a socket.%s\n", COL_RED, RESET;
		return 0;
	}
	...
	if (status < 0)
	{
		std::cerr << "%sERROR: Could not open a connection to host %s on port %d.%s\n", COL_RED, options->host, options->port, RESET;
	...
	if (options->starttls_ldap == true && tlsStarted == false)
	{
		...
		if (winStrnStr(buffer, ok, BUFFERSIZE)) {
#endif
...
#else
		else if (winStrnStr(buffer, unsupported, BUFFERSIZE)) {
#endif
			std::cerr << "%sSTARTLS LDAP connection to %s:%d failed with '%s'.%s\n",
				COL_RED, options->host, options->port, unsupported, RESET;
			return 0;
		}
		else {
			std::cerr << "%sSTARTLS LDAP connection to %s:%d failed with unknown error.%s\n",
				COL_RED, options->host, options->port, RESET;
			return 0;
		}
	}
	...
	if (options->starttls_psql == true && tlsStarted == false)
	{
		...
		if (buffer != 'S') {
			std::cerr << "%s    ERROR: server at %s:%d%s rejected TLS startup\n", COL_RED, options->host, options->port, RESET;
			return 0;
		}
	}
	...
	if (options->rdp == true && tlsStarted == false)
	{
		...
		if (readlen > sizeof(buffer)) {
			std::cerr << "%s    ERROR: unexpected data from %s:%d%s\n", COL_RED, options->host, options->port, RESET;
			return 0;
...
{
	char *charUserData = (char *)userdata;

	std::strncpy(buf, charUserData, size);
	// I don't know the semantics of these arguments, but it looks like this
	// could go badly wrong if userdata is too long.
	buf[strlen(charUserData)] = 0;
	return strlen(charUserData);
}
...
struct renegotiationOutput * newRenegotiationOutput(void)
{
	...
	myRenOut = (renegotiationOutput*)calloc(1, sizeof(struct renegotiationOutput));
	return(myRenOut);
}
...
int testCompression(struct sslCheckOptions *options, const SSL_METHOD *sslMethod)
{
	...
	if (socketDescriptor != 0)
	{
		...
		if (options->ctx != NULL)
		{
			if (SSL_CTX_set_cipher_list(options->ctx, "ALL:COMPLEMENTOFALL") != 0)
			{
				...
				if (status == true)
				{
					...
					{
						status = false;
						std::cerr << "%s    ERROR: Could create SSL object.%s\n", COL_RED, RESET;
					}
				}
			}
			else
			{
				status = false;
				std::cerr << "%s    ERROR: Could set cipher.%s\n", COL_RED, RESET;
			}
			// Free CTX Object
			SSL_CTX_free(options->ctx);
		}
		...
		{
			status = false;
			std::cerr << "%sERROR: Could not create CTX object.%s\n", COL_RED, RESET;
		}
	...
	{
		// Could not connect
		std::cerr << "%sERROR: Could not connect.%s\n", COL_RED, RESET;
		status = false;
		exit(status);
	}
...
int testFallback(struct sslCheckOptions *options, const SSL_METHOD *sslMethod)
{
	...
	SSL *ssl = NULL;
	BIO *cipherConnectionBio;
	const SSL_METHOD *secondMethod = NULL;
...
int testCipher(struct sslCheckOptions *options, const SSL_METHOD *sslMethod)
{
	...
	{
		winGetTimeOfDay(&tval_start, NULL);
	}

	// Create request buffer...
	memset(requestBuffer, 0, 200);
	_snprintf_s(requestBuffer, 199, 199, "GET / HTTP/1.0\r\nUser-Agent: SSLScan\r\nHost: %s\r\n\r\n", options->host);
	...
	if (socketDescriptor != 0)
	{
		if (SSL_CTX_set_cipher_list(options->ctx, options->cipherstring) != 0)
		{
			...
			if (ssl != NULL)
			{
				...
				{
					int msec;
					winGetTimeOfDay(&tval_end, NULL);
					winTimerSub(&tval_end, &tval_start, &tval_elapsed);
...
int main(int argc, char *argv[])
{
	...
	if ((xmlArg > 0) && (mode != mode_help))
	{
		...
		else
		{
			...
			{
				std::cerr << "%sERROR: Could not open XML output file %s.%s\n", COL_RED, argv[xmlArg] + 6, RESET;
				exit(0);
			}
		}
	...
	case mode_multiple:
		...
		else
		{
			if (fileExists(argv[options.targets] + 10) == true)
			{
				...
				if (targetsFile == NULL)
					std::cerr << "%sERROR: Could not open targets file %s.%s\n", COL_RED, argv[options.targets] + 10, RESET;
				else
				{
				...
				std::cerr << "%sERROR: Targets file %s does not exist.%s\n", COL_RED, argv[options.targets] + 10, RESET;
		}
